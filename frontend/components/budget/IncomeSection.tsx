 ...
 * - If taxed with custom rate: net = -gross * rate (deduction, e.g., lunch benefit)
 * - If taxed without custom rate: net = gross * (1 - defaultTaxRate)
 */
function calculateNetAmount(
  ...
  if (item.tax_percentage != null) {
    // Custom rate means it's a deduction (like lunch benefit)
    return -item.gross_amount * (item.tax_percentage / 100);
  }
  // Regular taxed income
  return item.gross_amount * (1 - defaultTaxRate / 100);
}

/**
 * Check if an income item is a deduction (has custom tax_percentage)
 */
function isDeduction(item: IncomeItem): boolean {
  return item.is_taxed && item.tax_percentage != null;
}

export function IncomeSection({ income, settings }: IncomeSectionProps) {
  ...
  const handleSave = (values: Record<string, string | number | boolean>) => {
    const data: IncomeFormData = {
      ...
      tax_percentage:
        values.tax_percentage && (values.tax_percentage as number) > 0
          ? (values.tax_percentage as number)
          : undefined,
    };
    ...
    <section className="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-800">
      ...
      <div className="divide-y divide-gray-100 dark:divide-gray-800">
        ...
        {income.map((item) => {
          ...
          const itemIsDeduction = isDeduction(item);
          return (
            <div
              key={item.id}
              ...
              <span className="text-gray-900 dark:text-gray-100">
                ...
                {item.is_taxed && (
                  <span className="ml-2 text-xs text-gray-500 dark:text-gray-400">
                    {itemIsDeduction
                      ? `(${formatPercentage(item.tax_percentage!)} deduction)`
                      : `(${formatPercentage(settings.tax_percentage)} tax)`}
                  </span>
                )}
              </span>
              <span className="text-right text-gray-700 dark:text-gray-300">
                {itemIsDeduction ? "—" : formatCurrency(item.gross_amount)}
              </span>
              <span
          ...
          <div className="grid grid-cols-3 px-4 py-3 bg-gray-50 dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
            ...
            <span className="text-right font-semibold text-gray-900 dark:text-gray-100">
              {formatCurrency(
                income.reduce(
                  (sum, item) =>
                    sum + calculateNetAmount(item, settings.tax_percentage),
                  0
                )
              )}
            </span>
